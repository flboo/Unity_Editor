...
public class MAP_sceneGizmoFunctions : EditorWindow
{
    ...
    {
        if (MAP_Editor.validTilePosition == false) return;
        if (MAP_Editor.pickToolOverride || MAP_Editor.selectTool == eToolIcons.pickTool)
            ...
            drawSceneGizmoCube(MAP_Editor.tilePosition, MAP_Editor.brushSize,
    ...
    public static void drawSceneGizmoCube(Vector3 position, Vector3 brushSize, Color gizmoColor)
    {
        ...
        // draw front
        Handles.DrawLine(position + new Vector3(-half.x, -scale, half.z),
            position + new Vector3(half.x, -scale, half.z));
        Handles.DrawLine(position + new Vector3(-half.x, -scale, half.z),
            position + new Vector3(-half.x, full.y - scale, half.z));
        Handles.DrawLine(position + new Vector3(half.x, full.y - scale, half.z),
            position + new Vector3(half.x, -scale, half.z));
        Handles.DrawLine(position + new Vector3(half.x, full.y - scale, half.z),
            position + new Vector3(-half.x, full.y - scale, half.z));
    ...
    public static void displayGizmoGrid()
    {
        ...
        {
            MAP_Editor.editorGameobject.GetComponent<MAP_GizmoGrid>().gridOffset =
                MAP_Editor.editorPreferences.gridOffset;
