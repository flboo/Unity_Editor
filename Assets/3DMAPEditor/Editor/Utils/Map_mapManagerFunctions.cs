...
public class Map_mapManagerFunctions
{
    ...
    {
        var mainmap = GameObject.Find(Define.CURRENT_MAP_NAME);
        if (mainmap == null)
    ...
    {
        var mainmap = new GameObject(mapName);
        for (var i = 1; i < 9; i++)
        {
            var layer = new GameObject(Define.LAYER + i);
    ...
    {
        if (MAP_Editor.gridSceneObject == null) MAP_Editor.gridSceneObject = GameObject.Find(Define.MAP_EDITOR_OBJECT);
