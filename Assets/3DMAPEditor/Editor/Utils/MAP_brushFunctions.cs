...
{
    private static readonly List<GameObject> brushTilesInParent = new();
    ...
    public static void updateBrushPosition()
    {
        if (MAP_Editor.brushTile != null)
        {
            ...
            if (MAP_Editor.currentBrushType == eBrushTypes.standardBrush)
                MAP_Editor.brushTile.transform.localScale = Vector3.Scale(
                    new Vector3(MAP_Editor.globalScale, MAP_Editor.globalScale, MAP_Editor.globalScale),
                    MAP_Editor.tileScale);
            else
            ...
            if (MAP_Editor.eraseToolOverride || MAP_Editor.pickToolOverride)
                foreach (var child in MAP_Editor.tileChildObjects)
    ...
    {
        if (MAP_Editor.selectTool != MAP_Editor.previousSelectTool) updateBrushTile();
    }
    ...
    public static void updateBrushTile(Vector3 tileScale)
    {
        ...
        {
            destoryBrushTile();
            MAP_Editor.brushTile = Instantiate(MAP_Editor.currentTile, Vector3.one, Quaternion.identity);
            MAP_Editor.brushTile.transform.eulerAngles =
                new Vector3(MAP_Editor.tileRotationX, MAP_Editor.tileRotation, 0);
            ...
            foreach (Transform child in MAP_Editor.brushTile.transform) child.gameObject.isStatic = false;
    ...
    public static void destoryBrushTile()
    {
        ...
        for (var i = 0; i < brushTilesInParent.Count; i++) DestroyImmediate(brushTilesInParent[i]);
    ...
    public static void cleanSceneOfBrushObjects()
    {
        if (MAP_Editor.findTileMapParent())
        {
            ...
            foreach (var obj in destoryList) DestroyImmediate(obj);
        }
    }
}